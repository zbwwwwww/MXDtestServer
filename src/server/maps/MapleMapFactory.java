...
public class MapleMapFactory {
    ...
    private static void loadLifeRaw(MapleMap map, int id, String type, int cy, int f, int fh, int rx0, int rx1, int x, int y, int hide, int mobTime, int team) {
        ...
        if (myLife instanceof MapleMonster) {
            ...
            int extraCopies = getExtraSpawnCopies(monster, mobTime, team);

            /* copy == 0 就是 wz 里的原始刷怪点，行为与改动前完全一致；
             * copy >= 1 是按 MOB_SPAWN_MULTIPLIER 复制出来的额外刷怪点 */
            for (int copy = 0; copy <= extraCopies; copy++) {
                MapleMonster spawnMob = monster;
                if (copy > 0) {
                    /* [MOB_DENSITY] 复制体沿 X 轴左右交替错开，避免全部叠在同一格。
                     * 偏移处下方必须有平台（findBelow），否则这一份直接跳过 —— 宁可少刷也不产生悬空怪。
                     * y 不用自己算：addMonsterSpawn() 内部会 calcPointBelow() 重新贴地。 */
                    int nx = x + mobSpawnOffset(copy);
                    MapleFoothold hold = map.getFootholds().findBelow(new Point(nx, y));
                    if (hold == null) {
                        continue;
                    }
                    spawnMob = new MapleMonster(id, monster.getStats());
                    spawnMob.setCy(cy);
                    spawnMob.setF(f);
                    spawnMob.setFh(hold.getId());
                    spawnMob.setPosition(new Point(nx, y));
                }

                if (mobTime == -1) { //does not respawn, force spawn once
                    map.spawnMonster(spawnMob);
                } else {
                    map.addMonsterSpawn(spawnMob, mobTime, team);
                }

                //should the map be reseted, use allMonsterSpawn list of monsters to spawn them again
                map.addAllMonsterSpawn(spawnMob, mobTime, team);
            }
        } else {
    ...
    /**
     * [MOB_DENSITY] 该刷怪点需要额外复制几份（0 = 原版行为）。
     *
     * 排除规则（宁可少刷，绝不出乱子）：
     *   1. mobTime < 0        -> 剧情/一次性怪（刷一次就不再出现），复制会重复触发
     *   2. team >= 0          -> 团队副本怪（怪物嘉年华等按队伍分配，复制会破坏机制）
     *   3. monster.isBoss()   -> BOSS 绝不复制，否则一张图刷出好几只 BOSS
     */
    private static int getExtraSpawnCopies(MapleMonster monster, int mobTime, int team) {
        int multiplier = YamlConfig.config.server.MOB_SPAWN_MULTIPLIER;
        if (multiplier <= 1 || mobTime < 0 || team >= 0 || monster.isBoss()) {
            return 0;
        }
        return multiplier - 1;
    }
    ...
    /**
     * [MOB_DENSITY] 第 copy 份复制体相对原始刷怪点的 X 轴偏移量。
     * copy: 1 => +45, 2 => -45, 3 => +90, 4 => -90, ...（左右交替，逐步远离原点）
     */
    private static int mobSpawnOffset(int copy) {
        int step = 45;
        int distance = (copy + 1) / 2;
        return ((copy % 2 == 1) ? 1 : -1) * distance * step;
    }

    public static MapleMap loadMapFromWz(int mapid, int world, int channel, EventInstanceManager event) {
        MapleMap map;

        MapleData mapData = mapSource.getData(getMapName(mapid));    // source.getData issue with giving nulls in rare ocasions found thanks to MedicOP
    ...
    /**
     * 地图 id 在 wz 里是否真的存在。
     * 给 GM 传送做前置校验用：loadMapFromWz 拿到 null 的 mapData 会抛 NPE，
     * 先查一次可以给出友好提示，而不是丢一个异常出去。
     */
    public static boolean mapExists(int mapid) {
        try {
            return mapSource.getData(getMapName(mapid)) != null;
        } catch (Exception e) {
            return false;
        }
    }
