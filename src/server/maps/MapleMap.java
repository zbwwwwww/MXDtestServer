...
public class MapleMap {
    ...
    public boolean damageMonster(final MapleCharacter chr, final MapleMonster monster, final int damage) {
        ...
        if (monster.isAlive()) {
            ...
            if (killed) {
                ...
                if (monster.getId() == 9300345 && chr.getQuest(21733).getProgress(21762).equals("2")) {
                    QuestScriptManager.getInstance().end(chr.getClient(), (short) 21733, 1204007, true);
                    QuestScriptManager.getInstance().getQM(chr.getClient()).setQuestProgress(21733, 9300345, 1);
                    QuestScriptManager.getInstance().getQM(chr.getClient()).completeQuest(21733);
                }
            }
            return true;
        }
    ...
    */
    ...
    public void removePlayer(MapleCharacter chr) {
        ...
        if (autoAttract && attractor == chr) {   // [吸怪模式] 吸引人离图 → 自动关
            stopAutoAttract();
        }
