    ...
    private void parse(WZIMGEntry entry, SeekableLittleEndianAccessor slea) {
        ...
        switch (marker) {
            ...
            case 8:
                ...
                // [FIX 修复解析错位] 原代码只处理 iMarker == 0 / 1，遇到 2、3 会进 else 分支：
                // 只打印一行日志，却没有消费它后面的字符串数据 ⇒ 读取指针从这一刻起错位，
                // 后面整棵 img 树全部解析错乱（表现为中文全变乱码）。
                // 按 wz 格式：2 与 0 等价（字符串内联在此处），3 与 1 等价（字符串在别处，用偏移量引用）。
                if (iMarker == 0 || iMarker == 2) {
                    entry.setData(WZTool.readDecodedString(slea));
                } else if (iMarker == 1 || iMarker == 3) {
