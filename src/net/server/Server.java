...
public class Server {
    ...
    public void forceUpdateCurrentTime() {
    ...
    public void init() {
        ...
        DatabaseConnection.initPool();   // [FIX 10] 幂等建池。★ 位置必须在 setDefault() 之后（与改之前的时序完全一致：
                                         //   时区 → 建连接 → 用连接），且必须在下面第一次 getConnection() 之前。
                                         //   首发时静态块已经建好 ⇒ 这里空转；服务端「重启」时它负责重建（静态块不会再跑）。
    ...
    public static void main(String[] args) {
        ...
        Server.getInstance().dumpSqlTask();
    ...
    private synchronized void shutdownInternal(boolean restart) {
        ...
        DatabaseConnection.closePool();   // [FIX 10] 关服收池（放在 if (!restart) 外面：重启也收）。
                                          //   重启路径下，紧接着的 getInstance().init() 里那行 initPool() 会重建，全程无窗口。
    ...
    public void dumpSqlTask() {
        ...
        timer.schedule(new TimerTask() {
            ...
            public void run() {
                ...
                //如下注释为linux版本
//                String savePath = "~/saves" + "/save_" + dtf.format(LocalDateTime.now()) + ".sql";
//                String cmd = "/bin/mysqldump" + " -u" + "root" + " -p" + "123456"
//                        + " --default-character-set=utf8 " + "heavenms" + " > " + savePath;
                String savePath = "saves/save_" + dtf.format(LocalDateTime.now()) + ".sql";
                String cmd = "\"C:\\Program Files\\MySQL\\MySQL Server 5.7\\bin\\mysqldump.exe\""
                        + " -u" + YamlConfig.config.server.DB_USER
                        + " --password=" + YamlConfig.config.server.DB_PASS
                        + " --default-character-set=utf8 heavenms > " + savePath;
                try {
