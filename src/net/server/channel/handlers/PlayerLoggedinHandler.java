...
public final class PlayerLoggedinHandler extends AbstractMaplePacketHandler {
    ...
    // diag: 进入游戏失败的每一处静默断开都必须留下痕迹，否则客户端只会干等、服务端一片安静
    private static String loginDiagStamp() {
        return "[" + new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new java.util.Date()) + "] ";
    }

    private static void logLoginFailure(MapleClient c, int cid, String reason) {
        String info;
        try {
            info = "cid=" + cid + " acc=" + c.getAccID() + " world=" + c.getWorld() + " channel=" + c.getChannel()
                    + " loggedIn=" + c.isLoggedIn() + " player=" + (c.getPlayer() == null ? "null" : c.getPlayer().getName());
        } catch (Throwable t) {
            info = "cid=" + cid + " (读取客户端状态时出错: " + t + ")";
        }
        FilePrinter.printError(FilePrinter.LOGIN_EXCEPTION, loginDiagStamp() + "PlayerLoggedinHandler 拒绝进入: " + reason + " | " + info);
    }
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (c.tryacquireClient()) { // thanks MedicOP for assisting on concurrency protection here
            try {
                ...
                if(wserv == null) {
                    logLoginFailure(c, cid, "world 不存在: " + c.getWorld());
                ...
                if(cserv == null) {
                    ...
                    if(cserv == null) {
                        logLoginFailure(c, cid, "channel 不存在: " + c.getChannel());
                        c.disconnect(true, false);
                        return;
                    }
                }
                ...
                if (player == null) {
                    ...
                    if (remoteHwid == null) {
                        logLoginFailure(c, cid, "拿不到 HWID");
                        c.disconnect(true, false);
                        return;
                    }
                } else {
                ...
                if (!server.validateCharacteridInTransition(c, cid)) {
                    logLoginFailure(c, cid, "角色转换校验失败(USE_IP_VALIDATION)");
                    c.disconnect(true, false);
                    return;
                }
                ...
                if (player == null) {
                    ...
                    if (player == null) { //If you are still getting null here then please just uninstall the game >.>, we dont need you fucking with the logs
                        logLoginFailure(c, cid, "loadCharFromDB 返回 null");
                        c.disconnect(true, false);
                        return;
                    }
                }
                ...
                */
                ...
                if (tryAcquireAccount(accId)) { // Sync this to prevent wrong login state for double loggedin handling
                    try {
                        ...
                        if (state != MapleClient.LOGIN_SERVER_TRANSITION || !allowLogin) {
                            logLoginFailure(c, cid, "登录状态不对: state=" + state + " (需要 " + MapleClient.LOGIN_SERVER_TRANSITION + ") allowLogin=" + allowLogin);
                    ...
                    logLoginFailure(c, cid, "该账号已有一次登录正在处理中(tryAcquireAccount 失败)");
                    c.setPlayer(null);
                    c.setAccID(0);
                ...
                Skill linkedSkill = SkillFactory.getSkill(10000000 * player.getJobType() + 12);   // linked character bonus skill, not present on every wz build
                if (linkedSkill != null) {
                    player.changeSkillLevel(linkedSkill, (byte) (player.getLinkedLevel() / 10), 20, -1);
                }
                ...
                FilePrinter.printError(FilePrinter.LOGIN_EXCEPTION, e, loginDiagStamp() + "PlayerLoggedinHandler 抛异常, cid=" + cid
                        + " acc=" + c.getAccID() + " world=" + c.getWorld() + " channel=" + c.getChannel()
                        + " player=" + (c.getPlayer() == null ? "null" : c.getPlayer().getName()));
                e.printStackTrace();
            } finally {
                c.releaseClient();
            }
        } else {
            logLoginFailure(c, cid, "客户端正被另一个线程处理(tryacquireClient 失败)");
            c.announce(MaplePacketCreator.getAfterLoginError(10));
        }
    }
