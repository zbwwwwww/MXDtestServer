...
public final class QuestActionHandler extends AbstractMaplePacketHandler {
    ...
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        System.out.println("action : " + action + " quest id : " + questid);
        if (action == 0) { // Restore lost item, Credits Darter ( Rajan )
            ...
            int npc = slea.readInt();
            System.out.println("action is 4 and npc is " + npc);
            if(!isNpcNearby(slea, player, quest, npc)) {
                System.out.println("action is 4 and npc is not here");
                return;
            }
            System.out.println("action is 4 and to start");
            if(quest.canStart(player, npc)) {
                System.out.println("action is 4 and can start");
            ...
            if(quest.canComplete(player, npc)) {
                QuestScriptManager.getInstance().end(c, questid, npc, false);
            }
        }
    }
}
