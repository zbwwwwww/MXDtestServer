...
public class MapleGuild {
    ...
    public MapleGuild(int guildid, int world) {
        ...
        // [FIX 连接泄漏] 原代码写的是 Connection con = null; try { con = ...; ... } catch ...
        // 但下面两个"早退"分支（公会不存在 / 公会查不到成员）直接 return，
        // 漏掉了 con.close() ⇒ 借出的连接没人还，只能等 HikariCP 的
        // leakDetectionThreshold(15s) 超时回收，并在日志里报
        //   "Connection leak detection triggered for com.mysql.jdbc.JDBC4Connection@..."
        //   at net.server.guild.MapleGuild.<init>(MapleGuild.java:75)
        //   at net.server.Server.getGuild ... client.MapleClient.disconnectInternal
        // （触发时机：关闭客户端时加载角色所在公会，公会字段为空/公会无成员即命中）
        // 改用 try-with-resources：正常结束、return、抛异常三条出口都会自动归还连接。
        try (Connection con = DatabaseConnection.getConnection()) {
            ...
            ps.close();
            rs.close();
            // con 由 try-with-resources 自动关闭（原来这里另有 con.close()，已移除避免重复）
