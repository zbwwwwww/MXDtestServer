...
public class NPCScriptManager extends AbstractScriptManager {
    ...
    public void action(MapleClient c, byte mode, byte type, int selection) {
        ...
        if (iv != null) {
            ...
            } catch (ScriptException | NoSuchMethodException | RuntimeException t) {
                // RuntimeException 也要接住。脚本里抛出的 NPE 之类如果逃出这里，
                // 下面的 dispose(c) 就不会执行，客户端会一直挂着一个死掉的 CM，
                // 表现出来就是"这个 NPC 再也点不动了"，只能重登客户端。
                if (getCM(c) != null) {
