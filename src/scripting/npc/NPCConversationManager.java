...
public class NPCConversationManager extends AbstractPlayerInteraction {
	...
	/**
	 * 按职业 id 转职。id 不存在时返回 false，角色不受影响。
	 * 返回值从 void 改成 boolean：现有脚本调用点全部忽略返回值，不受影响。
	 */
	public boolean changeJobById(int a) {
		MapleJob job = MapleJob.getById(a);
		if (job == null) {
			return false;
		}
	...
	public void maxMastery() {
		// 本服的 String.wz 里没有 Skill.img。原来的写法把那句 getData("Skill.img") 放在 try 外面，
		// 一旦拿到 null 就 NPE 逃出脚本引擎的 catch，导致 dispose() 不执行、这个 NPC 永久点不动。
		// 这里加空值保护，并且只保留一层 try。
		MapleData skillData = MapleDataProviderFactory.getDataProvider(new File(System.getProperty("wzpath") + "/" + "String.wz")).getData("Skill.img");
		if (skillData == null) {
			return;
		}
		for (MapleData skill_ : skillData.getChildren()) {
			try {
	...
	/**
	 * GM 菜单：把当前职业（含前置职业）的全部技能直接练到满级。
	 *
	 * @return 实际练满的技能数量
	 */
	public int maxJobSkills() {
		return getPlayer().maxJobSkills();
	}

	/**
	 * GM 菜单：学技能。level 小于等于 0 表示直接练到满级。
	 */
	public boolean giveSkill(int skillId, int level) {
		...
		int newLevel = (level <= 0) ? maxLevel : Math.min(level, maxLevel);
		getPlayer().changeSkillLevel(skill, (byte) newLevel, maxLevel, -1);
		return true;
	}

	/**
	 * GM 菜单：安全传送。地图 id 不存在时返回 false，角色原地不动。
	 */
	public boolean warpToMap(int mapId) {
		return getPlayer().warpTo(mapId);
	}
