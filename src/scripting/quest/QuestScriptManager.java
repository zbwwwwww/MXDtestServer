...
public class QuestScriptManager extends AbstractScriptManager {
    ...
    public void end(MapleClient c, short questid, int npc, boolean force) {
        MapleQuest quest = MapleQuest.getInstance(questid);
        if (!force) {
        ...
        try {
            ...
            if (c.canClickNPC()) {
                ...
                if ((!force) && !quest.hasScriptRequirement(true)) {
                    qm.dispose();
                    return;
                }
                ...
                NashornScriptEngine iv = getQuestScriptEngine(c, questid);
                if (iv == null) {
                    new Exception("END Quest " + questid + " is uncoded.").printStackTrace();
                    qm.dispose();
                    return;
                }
