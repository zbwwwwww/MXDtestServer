...
- **客户端分发（本仓库不包含客户端，请单独获取）**：客户端共 ~6.5 GB，`.wz` 资源占 6.5 GB，有 9 个文件超过 GitHub 单文件 100 MB 硬限（最大 `Character.wz` ≈1.2 GB）——直接 push 会被拒；只提交 EXE 和登录器.bat 也没用，客户端离开 `.wz` 资源无法运行。推荐做法：
  1. **GitHub Release 附件**：把 `GMS083_北冥整合版\MapleStoryHD` 打包分卷压缩（如 7z 卷 1.5GB×N），作为 Release 附件上传（单文件上限 2 GB）
  2. 或**网盘链接**（百度网盘/夸克等），README 留链接与提取码
  3. 仓库内保留一份 `tools/` 或文档说明客户端版本与校验方式，确保版本对齐

> 📦 **客户端下载**：（待填 —— 北冥 GMS083 整合版，GitHub Release 附件 / 网盘链接 + 提取码）
