...
| 召唤 | 召唤 BOSS（36 只 3 档）/ 召唤怪物-全级别段（13段781只，每段可全召或随机召5只） |
...
- 怪在围栏半径（15px）内自由游走，跑出边界即沿径向拉回围栏线（`MapleMap.startVacuumFence`）
- 每 80ms 扫描一次，越界即推回；地面护栏防止怪掉出平台
...
### 12. 群攻封包分批优化（防卡掉线）
- 群攻多目标时，血条更新/死亡动画/掉落三类封包不再瞬间同步发，改为逐只延迟发送
- 血条：50ms/个；死亡动画：300ms/个；掉落：100ms/个
- 修复 suppressDeathPkt 标志位误放金钱炸弹分支导致普通群攻未生效的bug
- BOSS召唤小怪改为分批（400ms/只，地图怪物上限30）；扎昆8条手臂分批召唤+500ms解封主体
- 批量召唤怪物改为每100ms召3只

### 13. GM菜单调整
- 删除"召唤怪物-点名"独立菜单，并入"全级别段"
- 段内首行改为"本段全部召唤"；分页第二页起加"返回上一页"按钮
- 删除"新增"栏目，功能按类别归入对应菜单
- 菜单顺序：移动/角色 → 怪物/地面 → 数值 → 职业 → 物品
- 新增"召唤所有标准怪物"：每段随机5只
- 标题署名：created by zhoubw
